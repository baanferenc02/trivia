...
public:
	...
	template <typename T,
			  typename detail::types::enable_if<detail::is_std_string<T>::value,
												bool>::type = true>
...
struct DOCTEST_INTERFACE ExpressionDecomposer {
	...
	// from GCC about "-Wparentheses" and since
...
#define DOCTEST_LOCK_MUTEX(name)                                               \
	const std::lock_guard<std::mutex> DOCTEST_ANONYMOUS(DOCTEST_ANON_LOCK_)(   \
		name);
...
struct DecisionPoint {
	// Number of branches available at this depth for the current traversal
	// path.
...
int Context::run()
{
	...
	if (!testArray.empty())
	{
		...
		else if (p->order_by.compare("rand", true) == 0)
		{
			...
			for (size_t i = testArray.size() - 1; i > 0; --i)
			{
				...
				// concurrency-mt-unsafe, misc-predictable-rand)
	...
	const bool query_mode =
		p->count || p->list_test_cases || p->list_test_suites;
	...
	for (auto& curr : testArray)
	{
		...
		// execute the test if it passes all the filtering
		{
			...
			do
			{ // NOLINT(cppcoreguidelines-avoid-do-while)
				// Reset per-run traversal data while keeping the current
	   ...
	   // is a static method)
				...
				const bool has_next_path =
					run_test ? p->traversal.advance() : false;
...
void JUnitReporter::test_run_end(const TestRunStats& p)
{
	...
	// Windows NOLINTNEXTLINE(misc-const-correctness)
